...
from hsflfm.calibration.vertices_organizing_functions import detect_all_irregular_points
...
def add_circles(irregular_points=None):
    ...
    irregular_set = (
        set(map(tuple, np.round(irregular_points, 4)))
...
all_irregulars = detect_all_irregular_points(
    vertices_dict, expected_spacing=expected_spacing
)
...
image = add_circles(
    irregular_points=all_irregulars.get((current_plane, current_camera), [])
)
im = ax.imshow(image)
fig.suptitle(get_title())
