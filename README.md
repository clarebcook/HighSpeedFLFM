...
Mention computer hardware and VS Code 
...
Note that ``conda-build`` must be installed in the base environment to use the ``develop`` command. 
...
> **Suggested demo:** We recommend running 'processing_scripts/process_from_pre_alignment.ipynb' to assess processing speed, and the .ipynb files in 'analysis_scripts' to view results. 
